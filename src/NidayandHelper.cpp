...
{
  this->_configfile = "/spiffs/config.json";
  this->_mqttclientid = ("ESPClient-" + String(chipID));
}
...
{
  return "/raw/esp8266/" + String(chipID) + "/" + type;
}
...
void NidayandHelper::mqtt_reconnect(PubSubClient &psclient, String uid, String pwd, std::list<const char *> topics)
{
  ...
  //     this->mqtt_publish(psclient, "/raw/esp8266/register", "{ \"id\": \"" + String(chipID) + "\", \"ip\":\"" + WiFi.localIP().toString() + "\"}");
